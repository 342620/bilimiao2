    ...
    fun deleteToview(aids: Set<Long>) = viewModelScope.launch(Dispatchers.IO) {
        try {
            ...
            if (res.code == 0) {
                ...
                A11yPopTip.show("已移除选中的${deleteItems.size}个视频")
                _selectedItemMap.clear()
                updateUserLibrary()
            } else {
                A11yPopTip.show(res.message)
    ...
    Column(
        ...
        Row(
            ...
            Row(
                modifier = Modifier
                    .clickable {
                        viewModel.changeAutoPlay(!viewModel.isAutoPlay)
                    }
                    .clearAndSetSemantics {
                        // 一个焦点、一条文案；开关状态只由 toggleableState 表达，避免重复朗读
                        contentDescription = "自动连播"
                        role = Role.Switch
                        toggleableState = if (viewModel.isAutoPlay) ToggleableState.On else ToggleableState.Off
                        onClick(label = null) {
                            viewModel.changeAutoPlay(!viewModel.isAutoPlay)
                            true
                        }
