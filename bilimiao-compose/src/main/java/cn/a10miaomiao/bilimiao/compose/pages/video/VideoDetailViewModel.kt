...
class VideoDetailViewModel(
    ...
    fun requestLike(
        ...
        try {
            ...
            e.printStackTrace()
            A11yPopTip.show(e.message ?: e.toString())
        }
    }
        ...
        when (item.key) {
            ...
            11 -> {
                ...
                if (current != -1) {
                    ...
                    A11yPopTip.show("已添加至下一个播放")
                } else {
                    A11yPopTip.show("添加失败，找不到正在播放的视频")
                }
            }
            12 -> {
                // 添加至最后一个播放
