...
//        }
    ...
    fun changedSpeed(newSpeed: Float) {
        if (speed != newSpeed) {
            ...
            A11yPopTip.show("已切换到${speed}倍速").showTop()
    ...
    fun changedQuality(newQuality: Int) {
        if (quality != newQuality) {
            lastPosition = views.videoPlayer.currentPositionWhenPlaying
            quality = newQuality
            A11yPopTip.show("正在切换清晰度").showTop()
        ...
        try {
            ...
            if (lastPosition > 0L) {
                ...
                controller.postPrepared(sourceInfo.lastPlayCid) {
                    A11yPopTip.show("自动恢复:$lastTimeStr", "重新开始")
                        .showTop()
                        .showLong()
                        .setButton { dialog, v ->
            ...
            if (isChangedQuality) {
                if (sourceInfo.quality == quality) {
                    A11yPopTip.show("已切换至【${sourceInfo.description}】").showTop()
                } else {
                    A11yPopTip.show("清晰度切换失败").showTop()
                }
            } else {
        ...
        playerCoroutineScope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    A11yPopTip.show(e.message.toString()).showTop()
                }
            }
        }
    }
    ...
    override fun openPlayer(source: BasePlayerSource) {
        ...
        playerCoroutineScope.launch(Dispatchers.Main) {
            ...
            // 播放倍速提示
            if (speed != 1f) {
                A11yPopTip.show("注意，当前为${speed}倍速").showTop()
            }
        }
        // 播放器是否默认全屏播放
